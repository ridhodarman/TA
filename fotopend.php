...
$sql=pg_query("SELECT * FROM educational_building");
while ($data=pg_fetch_assoc($sql)) {
	...
	while ($dataa=pg_fetch_assoc($sql2)) {
		...
		$sql4 = pg_query("INSERT INTO educational_building_gallery (educational_building_id, photo_url, upload_date) VALUES ('$id_bang', '$url', '$tgl')");
		if ($sql4){
			echo "sukses<br>";
		}
		else {
